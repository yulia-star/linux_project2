...
Политика по умолчанию (policy ACCEPT - отвечает на пакеты или policy DROP - не отвечает на пакеты или policy REJECT - предполагает, что наш компьютер отвечает на каждый пакет хоть и отказом, но это расходует ресурс)
...
Сеть:
![part5_network](https://github.com/yulia-star/linux_project2/blob/main/part5_network.png)
